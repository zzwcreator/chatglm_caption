    ...
    while 1:
        if Rece_flag == 1:
            ...
            if fun_flag == 1:
                ...
                print("Frame saved as image.png") 
                res = sample_dense_captions_image_file()
                Prompt = "请根据图像的dense caption进行视觉问答,要求只输出答案并不超过50字。 问题：" + Ques + "图像的dense caption为：" +str(res)
            elif fun_flag == 3:
                frame = cap.read()
                cv2.imwrite('image.png', frame)  
                print("Frame saved as image.png") 
                res = sample_dense_captions_image_file()
                Prompt = "请理解我问题的意图，并根据图像的dense caption进行视觉理解问答,要求只输出答案并不超过50字。 问题：" + Ques + "图像的dense caption为：" +str(res)
